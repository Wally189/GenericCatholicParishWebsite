...
const siteMap = [
  ...
  },
  {
    ...
    children: [
      ...
      { label: "Refugee / Outreach Work", href: "parish-life.html#refugee-borderlands" },
  ...
  },
  {
    ...
    children: [
      ...
      { label: "Patron Saint / Dedication", href: "about-the-parish.html#patron-saint" },
...
function renderHomePage(homeContent, parishSettings, newsContent, newsletterContent) {
  ...
  contactHost.append(
    ...
    createInfoCard({
      ...
      links: [
        { label: `Call parish priest: ${parishSettings.contact.priestPhone}`, url: buildPhoneHref(parishSettings.contact.priestPhone) },
        { label: `Emergency mobile: ${parishSettings.contact.priestEmergency}`, url: buildPhoneHref(parishSettings.contact.priestEmergency) },
        { label: "Email parish priest", url: `mailto:${parishSettings.contact.priestEmail}` }
      ]
    }),
    createInfoCard({
      meta: "Safeguarding",
...
function renderParishLifePage(pageContent, parishSettings) {
  ...
  renderPageIntro(pageContent.intro, (aside) => {
    aside.append(
      createNode("p", { className: "eyebrow", text: pageContent.intro.asideMeta || "" }),
      createNode("h3", { text: "Outreach Partner" }),
      createNode("p", { text: pageContent.intro.asideBody || "" })
    );
    aside.append(createLink("Outreach partner website", "https://example.org/outreach-partner"));
    aside.append(createLink("Email outreach partner", `mailto:${parishSettings.contact.borderlandsEmail}`));
  });
...
function renderSafeguardingPage(pageContent, parishSettings) {
  ...
  renderCardGrid([
    ...
    },
    {
      ...
      links: [
        { label: "Diocesan safeguarding page", url: parishSettings.safeguarding.diocesanUrl },
        { label: "Email diocesan safeguarding", url: `mailto:${parishSettings.safeguarding.diocesanEmail}` }
      ]
    },
    {
...
function renderLegalPage(legalContent) {
  ...
  setBackgroundImage(hero, "/uploads/hero-interior.svg");
